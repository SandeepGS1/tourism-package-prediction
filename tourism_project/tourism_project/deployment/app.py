...
st.write("App predicts whether a customer is likely to purchase the Tourism Package.")
st.write(
    "Enter all the below details to get the prediction")
...
# Predict button
if st.button("Predict Tourism Result", type="primary"):
    ...
    if prediction == 1:
        st.success("This customer is likely to take tourism!")
        st.metric("Purchase Probability", f"{prediction_proba:.1%}")
    else:
        st.warning("This customer is unlikely to take tourism.")
        st.metric("Purchase Probability", f"{prediction_proba:.1%}")
